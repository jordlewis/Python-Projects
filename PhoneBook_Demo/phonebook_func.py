...
# Importing other modules to establish access
...
def create_db(self):
    ...
    with conn:
        ...
        # Committing to save changes and close the database connection
        conn.commit()
    conn.close()
    first_run(self)
...
def onDelete(self):
    var_select = self.lstList1.get(self.lstList1.ourselection()) # Listbox's selected value
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
        # Check count to ensure that this is not the last record in
        # the database...cannot delete the last record or will throw error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
...
def onUpdate(self):
    ...
    var_phone = self.txt_phone.get().strip() # normalize the data to maintin database integrity
    var_email = self.txt_email.get().strip()
        ...
        with conn:
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    messagebox.showinfo("Cancel request","No other changes have been made to ([])".format(var_value))
            else:
                messagebox.showinfo("No changes detected","Both ({}) and ({}) \nalready exists in the database for the name. \n\nYour update request has been cancelled.".format(var_phone,var_email))
            onClear(self)
        conn.close()
    else:
        messagebox.showerror("Missing information","Please select a new name from the list. \nThen edit the phone or email information.")
    onClear(self)
