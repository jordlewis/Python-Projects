...
def create_account(request):
    ...
    # Checks if request method is POST
    if request.method == 'POST':
        if form.is_valid():  # Check to see if the submitted form is valid if so, saves the form
            form.save()  # Saves new account
            return redirect('index')  # Returns user back to the home page
    content = {'form': form}  # Saves content to the template as the dictionary
    # adds content of form to page
...
def balance(request, pk):
    ...
    for t in transactions:  # Loop through transactions and determine which is a deposit or a withdrawal
        if t.type == 'Deposit':
            current_total += t.amount  # If deposit add money to balance
...
def transaction(request):
    ...
    # Checks if request method is POST
    if request.method == 'POST':
        if form.is_valid():  # Check if the submitted form is valid and if so, saves the form
            pk = request.POST['account']  # Retrieve which account the transaction was for
            form.save()  # Save s the transaction form
